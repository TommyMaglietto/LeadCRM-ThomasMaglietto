...
export function KanbanColumn({ stage, leads, onLeadClick }: KanbanColumnProps) {
  ...
  return (
    <div className="flex flex-col shrink-0 w-[220px] rounded-[8px] border border-ink/10 bg-cream-dark overflow-hidden">
      {/* Column header bar */}
      <div
      ...
      >
        {leads.length === 0 ? (
          ...
          >
            <span className="text-xs text-rubble">Drop here</span>
          </div>
        ) : (
          leads.map((lead) => (
            <KanbanCard
